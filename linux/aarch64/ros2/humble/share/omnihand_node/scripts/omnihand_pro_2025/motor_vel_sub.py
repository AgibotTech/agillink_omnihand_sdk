...
@Date: 2025-11-06
@Description: Python node to subscribe to motor velocity feedback (Omnihand2025Pro)
...
try:
    from omnihand_pro_2025_node_msgs.msg import MotorVel
except ImportError:
...
class MotorVelSubscriber(Node):
    def __init__(self, hand_side):
        ...
        self.subscription = self.create_subscription(
            MotorVel,
            f'/omnihand/omnihand_pro_2025/{hand_side}/motor_vel',
            self.motor_vel_callback,
            10
        )

        self.get_logger().info(f'{hand_side.capitalize()} Motor Velocity Subscriber Node started (O12, 12 DOF)')
