...
void printUsage(const char* program_name) {
  ...
  AgilinkLogger::get().infof(TAG, "  left   - Control left hand only (can0)");
  AgilinkLogger::get().infof(TAG, "  right  - Control right hand only (can1)");
...
int main(int argc, char** argv) {
  ...
  AgilinkLogger::get().infof(TAG, "============================================");
  AgilinkLogger::get().infof(TAG, "OmniHand 3 Ultra M (O20) - SocketCAN Control");
