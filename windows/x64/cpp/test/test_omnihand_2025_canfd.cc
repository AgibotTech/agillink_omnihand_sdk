...
TEST_F(OmniHand2025CanfdTest, GetNumOfTactilePoints) {
  ...
  for (auto finger : hand_->GetSensorOrder()) {
    ...
    AgilinkLogger::get().infof(TAG, "[GetNumOfTactilePoints] %s: %zu points", agilink::omnihand::ToString(finger).c_str(), pts);
...
TEST_F(OmniHand2025CanfdTest, GetLenOfTactileDatum) {
  ...
  for (auto finger : hand_->GetSensorOrder()) {
    ...
    AgilinkLogger::get().infof(TAG, "[GetLenOfTactileDatum] %s: %zu bytes/point", agilink::omnihand::ToString(finger).c_str(), len);
...
TEST_F(OmniHand2025CanfdTest, GetNumOfRepliedTactileFrames) {
  ...
  for (auto finger : hand_->GetSensorOrder()) {
    ...
    AgilinkLogger::get().infof(TAG, "[GetNumOfRepliedTactileFrames] %s: %zu frame(s)", agilink::omnihand::ToString(finger).c_str(), frames);
...
TEST_F(OmniHand2025CanfdTest, GetSNOfTactileSensor) {
  ...
  for (auto finger : hand_->GetSensorOrder()) {
    ...
    AgilinkLogger::get().infof(TAG, "[GetSNOfTactileSensor] %s: \"%s\"", agilink::omnihand::ToString(finger).c_str(), sn.c_str());
  }
