...
int main(int argc, char** argv) {
  ...
  AgilinkLogger::get().infof(TAG, "============================================");
  AgilinkLogger::get().infof(TAG, "OmniHand 2025 - SocketCAN Control");
  ...
  if (mode == "right" || mode == "both") {
    ...
    if (mode == "right") {
...

      {
        ...
        AgilinkLogger::get().infof(TAG, "Left Hand Angles (rad, %s): [%s]", left_interface.c_str(), msg.c_str());
      }

      {
        ...
        AgilinkLogger::get().infof(TAG, "Right Hand Angles (rad, %s): [%s]", right_interface.c_str(), msg.c_str());
      }
    }
  }
