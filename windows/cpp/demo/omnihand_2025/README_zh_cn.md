# OmniHand 2025 SDK C++ 示例

本目录包含 OmniHand 2025 SDK 的 C++ 使用示例，展示了不同通信方式的使用方法。
...
**适用场景**：
- 使用 RS485 串口通信
- 需要修改代码中的串口路径（例如 `/dev/ttyUSB0` 或 `COM3`）

**注意**：
- Linux: 确保有串口访问权限 `sudo chmod 666 /dev/ttyUSB0`
- Windows: 使用 COM 端口名称，例如 `COM3`
- `both` 模式需要两个不同的串口

### 4. `O10_demo_socketcan.cc` (仅 Linux)
**SocketCAN 通信**
...
# 运行示例
./demo_omnihand_2025_socketcan left    # 使用 can0
./demo_omnihand_2025_socketcan right   # 使用 can0
./demo_omnihand_2025_socketcan both    # 使用 can0 和 can1
```
...
| **RS485** | 串口通信，兼容性好 | 速度较慢，需要串口权限 | 串口环境，兼容性要求 |
...
4. **权限问题**：
   - RS485: Linux 需要串口访问权限
   - SocketCAN: 需要 root 权限配置接口
5. **频率控制**：SDK 内部已实现请求频率控制（默认 10 Hz），无需手动添加延迟

## 更多信息

- [API 文档](../../../doc/zh_cn/API_CPP_O10.md)
