...
| `joint_cmd_source` | `std_msgs/Int8` | 订阅 (你发布) | — | 选择命令源：`0=NORMAL`、`1=ANGLE`、`2=POSITION` |
| `joint_angle_cmd` | `sensor_msgs/JointState` | 订阅 (你发布) | 命令源 `1` | 角度覆盖，`position[]` = rad |
| `joint_position_cmd` | `std_msgs/Int16MultiArray` | 订阅 (你发布) | 命令源 `2` | 电机原始位置覆盖 |
| `joint_position_states` | `omnihand_node_msgs/Int16MultiArrayStamped` | 发布 (你订阅) | 接收有效 `joint_position_cmd` 时 | 带时间戳的电机原始位置回读 |
| `joint_mix_control_cmd` | `sensor_msgs/JointState` | 订阅 (你发布) | — | 位置+力矩混合控制（仅 O10/O12，见下文） |
| `joint_error_cmd` | `std_msgs/Empty` | 订阅 (你发布) | — | 触发错误码查询 |
| `joint_error_states` | `omnihand_node_msgs/Int16MultiArrayStamped` | 发布 (你订阅) | 收到 `joint_error_cmd` 时 | `header.stamp` + 错误码 `data[]` |
| `joint_temperature_cmd` | `std_msgs/Empty` | 订阅 (你发布) | — | 触发温度查询 |
| `joint_temperature_states` | `omnihand_node_msgs/Int16MultiArrayStamped` | 发布 (你订阅) | 收到 `joint_temperature_cmd` 时 | `header.stamp` + 温度 `data[]` |
| `joint_current_cmd` | `std_msgs/Empty` | 订阅 (你发布) | — | 触发电流查询 |
| `joint_current_states` | `omnihand_node_msgs/Int16MultiArrayStamped` | 发布 (你订阅) | 收到 `joint_current_cmd` 时 | `header.stamp` + 电流 `data[]` |
| `joint_control_mode_cmd` | `std_msgs/Int8MultiArray` | 订阅 (你发布) | — | 写入控制模式 `data[]`（仅 O12/H3U_M） |
| `joint_control_mode_states` | `omnihand_node_msgs/Int8MultiArrayStamped` | 发布 (你订阅) | 收到 `joint_control_mode_cmd` 时 | 带时间戳的控制模式回读（仅 O12/H3U_M） |
| `joint_voltage_cmd` | `std_msgs/Int16MultiArray` | 订阅 (你发布) | — | 写入电压指令 `data[]`（仅 O12） |
| `joint_voltage_states` | `omnihand_node_msgs/Int16MultiArrayStamped` | 发布 (你订阅) | 收到 `joint_voltage_cmd` 时 | 带时间戳的电压回读（仅 O12；O12 固件 <= 1.2.15 不可用） |
| `joint_current_threshold_cmd` | `std_msgs/Int16MultiArray` | 订阅 (你发布) | — | 写入电流阈值 `data[]` |
| `joint_current_threshold_states` | `omnihand_node_msgs/Int16MultiArrayStamped` | 发布 (你订阅) | 收到 `joint_current_threshold_cmd` 时 | 带时间戳的电流阈值回读 |
...
状态 topic 使用 `omnihand_node_msgs/Int16MultiArrayStamped` 或 `Int8MultiArrayStamped`，其中包含 `header`、`layout` 和 `data`；订阅者可从 `header.stamp` 获取采集时间。命令 topic 仍使用 `std_msgs/Int16MultiArray` 或 `std_msgs/Int8MultiArray`。`joint_states` 仍为 `sensor_msgs/JointState`，同样包含 `header.stamp`。触觉数据继续使用各产品 `omnihand_*_node_msgs` 类型。
...
- `ros2_joint_cmd_demo.cpp` — 使用 `sensor_msgs/JointState` 发送位置指令，并使用带时间戳的 `omnihand_node_msgs` 数组读取温度和电流
