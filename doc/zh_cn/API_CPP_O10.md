...
**OmniHand 2025 (O10)** 是一�?10 自由度灵巧手，配�?1D 触觉传感器。本文档描述了用于控制和交互 OmniHand 2025 设备�?C++ API�?
...
struct TactileSensorData {
    Finger sensor_id_;           // 传感�?ID（手�?手心/手背�?
    std::vector<uint8_t> data_;   // 传感器数据（单位�?g，最大值：255g�?
};
```
 ...
 * @return OmniHand2025 实例的唯一指针，如果找不到设备则返�?nullptr
 */
static std::unique_ptr<OmniHand2025> createHandByZlgcan(
    HandType hand_type,
...
/**
 * @brief 检查初始化状�?
 * @return 如果初始化成功返�?true，否则返�?false
 */
bool Init() const;
```
...
/**
 * @brief 获取厂商信息�?
 * @return 包含产品型号、序列号、硬件版本、软件版本等�?VendorInfo 结构�?
 */
VendorInfo GetVendorInfo() const;

/**
 * @brief 获取设备信息�?
 * @return 包含设备 ID 和通信参数�?DeviceInfo 结构�?
...
/**
 * @brief 从广播地址（hand_device_id = 0x00）获取设备信�?
 * @param canfd_device_id USB CANFD 适配器设备索�?
 * @param canfd_channel_id CAN 通道索引（默认：0）。双通道适配�?USBCANFD-200U): can0=0, can1=1；单通道适配�?USBCANFD-100U): 始终�?
 * @return DeviceInfo 结构，如果请求失败则返回空的 DeviceInfo
 * @note 此函数发送广播请求以发现 CAN 总线上的设备
 * @note 仅适用�?CAN 通信，RS485 不支�?
 */
...
/**
 * @brief 从广播地址（hand_device_id = 0x00）通过序列号获取设备信�?
 * @param usbcanfd_serial_number USB CANFD 设备序列号（支持部分匹配�?
 * @param canfd_channel_id CAN 通道索引（默认：0）。双通道适配�?USBCANFD-200U): can0=0, can1=1；单通道适配�?USBCANFD-100U): 始终�?
...
| 索引 | 关节名称         | 最小角�?(rad) | 最大角�?(rad) | 最小角�?(°) | 最大角�?(°) | 速度限制 (rad/s) |
| ---- | ---------------- | ------------- | ------------- | ----------- | ----------- | --------------- |
...
/**
 * @brief 设置所有主动关节的角度�?
 * @param angles 关节角度向量（单位：弧度）。必须包�?10 个值�?
 ...
 * @return 所有关节角度向量（单位：弧度）。返�?16 个值（10 个主�?+ 6 个被动）�?
 * @note �?10 个值为主动关节（顺序参考上表），后 6 个值为被动关节�?
 */
std::vector<double> GetAllJointAngles() const;

/**
 * @brief 从主动关节角度计算所有关节角度（包括主动和被动关节）�?
 * @param active_joint_pos 主动关节角度向量（单位：弧度）。必须包�?10 个值�?
...
/**
 * @brief 设置单个关节电机的位置�?
 * @param joint_motor_index 关节电机索引�?-10）�?
...
/**
 * @brief 批量获取所有关节电机的位置�?
 * @return 当前位置向量。返�?10 个值，每个值在 0-4096 范围内�?
 */
...
OmniHand 2025 (O10) 使用 **1D 触觉传感�?*，具有以下特性：
- **数据单位**�?g
- **最大�?*�?55g
- **采样频率**�?0Hz
- **传感器位�?*：手指（每个 16 个点）、手心（78 个点）、手背（102 个点�?
 ...
 * @note 数据单位�?g，最大值：255g，采样频率：10Hz
 *       - 手指：返�?16 个数据点，每个传感器点一�?
 *       - 手心：返�?25 个数据点，每 3 个传感器点一个（降采样）
 *       - 手背：返�?25 个数据点，每 4 个传感器点一个（降采样）
 */
std::vector<uint8_t> GetTactileSensorData(Finger eFinger) const;

/**
 * @brief 一次性获取所�?1D 触觉传感器原始数据�?
...
**⚠️ 重要建议：获取多个传感器数据时，强烈推荐使用 `GetAllTactileSensorDataRaw()` 而不是循环调�?`GetTactileSensorDataRaw()`�?*

以下表格对比了两种方式的差异�?

| 方面 | 循环调用 `GetTactileSensorDataRaw()` (7�? | 使用 `GetAllTactileSensorDataRaw()` (1�? |
|------|-------------------------------------------|-------------------------------------------|
| **请求间隔累积** | 7 × interval_ms (例如�? × 3ms = 21ms) | 1 × interval_ms (例如�? × 3ms = 3ms) |
| **独立超时检查次�?* | 7次（每次请求都有独立超时风险�?| 1次（多帧接收在单次请求内完成�?|
| **CAN总线占用** | 7次请�?+ 7次响�?= 14次帧传输 | 1次请�?+ 5次响�?= 6次帧传输 |
 ...
 * @param joint_motor_index 关节电机索引�?-10）�?
 * @param mode 控制模式枚举值�?
 * @note 纯力控模�?(TORQUE) 不支持。请使用混合控制模式（POSITION_TORQUE, VELOCITY_TORQUE, POSITION_VELOCITY_TORQUE）�?
 */
 ...
 * @param ctrl_modes 控制模式向量。必须包�?10 个值�?
 * @note 串口通信（RS485）不支持此接口�?
 */
 ...
 * @note 纯力控模�?(TORQUE) 不支持。请使用混合控制模式�?
 * @note 串口通信（RS485）不支持此接口�?
 */
...
**注意**：OmniHand 2025 (O10) 不支持设置温度上报周期。此功能仅适用�?OmniHand Pro 2025 (O12)�?
...
**注意**：OmniHand 2025 (O10) 不支持设置电流上报周期。此功能仅适用�?OmniHand Pro 2025 (O12)�?
...
int main() {
    ...
    if (!hand || !hand->Init()) {
        std::cerr << "初始�?OmniHand 2025 失败" << std::endl;
        return -1;
    }
    ...
    std::cout << "拇指传感器数�? " << thumb_data.size() << " 个点" << std::endl;
...
- [OmniHand 2025 (O10) 运动学求解器 C++ API](API_KINEMATICS_CPP_O10.md) - 运动学计�?
